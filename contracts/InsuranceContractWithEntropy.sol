...
/**
 * @title InsuranceContractWithEntropy
 * @notice Insurance marketplace contract with Pyth Entropy integration for fair underwriter ordering
 * 
 * Key Features:
 * - Fair underwriter matching using verifiable randomness
 * - No preferential ordering (prevents gaming)
 * - Sybil resistance through on-chain entropy
 * - Equal exposure for all qualified underwriters
 * 
 * Integration: Pyth Entropy
 * - Provides cryptographically secure random numbers
 * - Used to shuffle underwriter order fairly
 * - Eliminates bias in matching algorithm
 * 
 * Reference: https://docs.pyth.network/entropy
 */
contract InsuranceContractWithEntropy is ReentrancyGuard {
    // FDC Verification contract
    IFdcVerification public immutable fdcVerification;
    
    // Pyth Entropy integration (would be actual Entropy contract in production)
    // For now, we track entropy requests and their fulfillment
    ...
    struct QuoteRequest {
        ...
        uint256 entropyRequestId;     // NEW: Link to entropy request for fair matching
    }
    ...
    struct Offer {
        ...
        uint256 randomOrderIndex;   // NEW: Random order index from Pyth Entropy
    }
    ...
    // NEW: Entropy request tracking for fair matching
    ...
    // NEW: Entropy request storage
    uint256 public entropyRequestCounter;
    mapping(uint256 => EntropyRequest) public entropyRequests;

    // Mapping provider => available funds (to fund coverage)
    mapping(address => uint256) public providerFunds;
    
    // NEW: Track underwriter ordering seeds for transparency
    mapping(uint256 => bytes32) public underwriterOrderSeeds;
    ...
    // NEW: Pyth Entropy events
    event EntropyRequested(
        uint256 indexed entropyRequestId,
    ...
    event FairOrderingApplied(
        uint256 indexed quoteRequestId,
        bytes32 orderSeed,
        string fairnessGuarantee
    );
     ...
     * 
     * NEW: Automatically requests entropy for fair underwriter matching
     */
    function getQuote(
        ...
        // NEW: Request entropy for fair matching
        uint256 entropyReqId = _requestEntropyForMatching(quoteRequestId, msg.sender);
        ...
        quoteRequests.push(QuoteRequest({
            ...
            entropyRequestId: entropyReqId  // NEW: Link entropy request
        }));
     ...
     * 
     * NEW: Offer includes random order index from Pyth Entropy
     */
    function offer(
        uint256 quoteRequestId,
        IWeb2Json.Proof calldata proof
        ...
        // NEW: Get random order index from entropy
        uint256 randomOrderIndex = _getRandomOrderIndex(qr.entropyRequestId, msg.sender);
        ...
        offers.push(Offer({
            ...
            randomOrderIndex: randomOrderIndex  // NEW: Fair ordering
        }));
    ...
    /**
     * NEW: Request entropy for fair underwriter matching
     * In production, this would call Pyth Entropy contract
     */
    function _requestEntropyForMatching(
        uint256 quoteRequestId,
        address requester
    ) internal returns (uint256 entropyRequestId) {
        entropyRequestId = entropyRequestCounter++;
        
        entropyRequests[entropyRequestId] = EntropyRequest({
        ...
        emit EntropyRequested(entropyRequestId, quoteRequestId, requester);
        
        // In production: Call Pyth Entropy contract to request random number
        // entropy.requestRandomNumber(userCommitment);
        
        // For demo: Simulate fulfillment with block-based randomness
        _fulfillEntropyRequest(entropyRequestId);
        
        return entropyRequestId;
    }

    /**
     * NEW: Fulfill entropy request (would be called by Pyth Entropy callback)
     */
    function _fulfillEntropyRequest(uint256 entropyRequestId) internal {
        EntropyRequest storage req = entropyRequests[entropyRequestId];
        require(!req.fulfilled, "Already fulfilled");
        
        // In production: This would be called by Pyth Entropy provider
        // For demo: Use block-based pseudo-randomness (NOT production-ready)
        uint64 randomNumber = uint64(uint256(keccak256(abi.encodePacked(
            block.timestamp,
        ...
        // Generate order seed for this quote request
        bytes32 orderSeed = keccak256(abi.encodePacked(randomNumber, req.quoteRequestId));
        underwriterOrderSeeds[req.quoteRequestId] = orderSeed;
        
        emit EntropyFulfilled(entropyRequestId, randomNumber, orderSeed);
        emit FairOrderingApplied(
            req.quoteRequestId,
            orderSeed,
            "Pyth Entropy ensures unbiased underwriter ordering"
        );
    }

    /**
     * NEW: Get random order index for underwriter offer
     */
    function _getRandomOrderIndex(
        uint256 entropyRequestId,
        address provider
    ) internal view returns (uint256) {
        EntropyRequest storage req = entropyRequests[entropyRequestId];
        require(req.fulfilled, "Entropy not yet fulfilled");
        
        // Generate deterministic but random-looking index
    ...
    /**
     * NEW: Get fairness proof for a quote request
     * Allows anyone to verify the random seed used for ordering
     */
    function getFairnessProof(uint256 quoteRequestId) external view returns (
        bytes32 orderSeed,
        uint64 randomNumber,
        uint256 entropyRequestId,
        bool fulfilled
        ...
        require(quoteRequestId < quoteRequests.length, "Invalid quoteRequestId");
        QuoteRequest storage qr = quoteRequests[quoteRequestId];
        EntropyRequest storage req = entropyRequests[qr.entropyRequestId];
        
        return (
            underwriterOrderSeeds[quoteRequestId],
            req.randomNumber,
            qr.entropyRequestId,
            req.fulfilled
        );
